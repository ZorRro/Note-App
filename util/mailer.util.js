...
var messages = {};
messages.activationMessage = `Click on <a href="${APP_URL}/auth/activate?identity=<identity>&info=<info>">this</a> link to activate user.<br /><br />`;
messages.resetPasswordMessage = `Click on <a href="${APP_URL}/auth/login?identity=<identity>&info=<info>">this</a> link to reset password.<br /><br />`;
...
module.exports.buildEmail = function(
  ...
  if (type === "activation") {
    var message = messages.activationMessage
      .replace("<identity>", userId)
      .replace("<info>", token);
    var subject = "Web App: Activation Link";
  } else if (type === "reset") {
    var message = messages.resetPasswordMessage
      .replace("<identity>", userId)
      .replace("<info>", token);
    var subject = "Web App: Reset Password";
  }
